...
|   조원 이름	| 회고  	 |
|---	|-------|
|   **김윤지**   | 이번 프로젝트는 개발자로서 어떻게 성장하고 협력해야 하는지에 대한 깊은 깨달음을 얻은 시간이었습니다. 첫 프로젝트와 달리 MyBatis, JPA, Spring Security 등 낯선 기술들을 마주했을 때는 '과연 잘 해낼 수 있을까' 하는 걱정이 앞섰지만, 이내 새로운 도전에 대한 기대감으로 바뀌었습니다. 프로젝트 초반, DDD와 ERD 설계에 많은 시간을 쏟았습니다. 당장의 개발 진행이 더뎌지는 것 같아 조급한 마음도 들었지만, 탄탄한 설계가 후반부 개발 과정에서 얼마나 큰 힘이 되는지를 모두가 체감할 수 있었습니다. 덕분에 복잡한 비즈니스 로직을 도메인 중심으로 사고하는 법을 배웠고, 개발 중 테이블 구조를 여러 번 수정하는 어려움 속에서도 방향을 잃지 않을 수 있었습니다. 개발 과정에서는 Github 협업 규칙을 세우고, 각자 회원 관리, SNS 기능 등을 구현하며 이론으로만 알던 기술들을 체화했습니다. 특히 Spring Security의 토큰 발급이나 JPA 엔티티 매핑의 수많은 오류 로그 앞에서 좌절하기도 했지만, 혼자가 아닌 팀원들과 함께 머리를 맞대고 해결해나가며 '함께 일하는 즐거움'과 값진 협업 경험을 얻었습니다. 이번 프로젝트를 통해 얻은 가장 큰 수확은 '할 수 있다'는 자신감입니다. 이 경험을 발판 삼아 다음 프로젝트 전까지 코드 리팩토링을 진행하여, 한 단계 더 발전한 개발자의 모습으로 나아갈 것입니다. |
