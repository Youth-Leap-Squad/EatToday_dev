...
|   조원 이름	| 회고  	 |
|---	|-------|이번 술/안주 페어링 추천 웹프로젝트는 제게 개발자로서 한 단계 성장할 수 있는 중요한 경험이었습니다. DDD, ERD, MyBatis, JPA, MSA 등 낯설고 방대한 기술들을 실제로 적용하면서 처음에는 막막했지만, 시간이 지날수록 두려움이 자신감으로 바뀌었습니다. 특히 게시글 기능을 담당하며 CRUD를 구현하는 과정에서 많은 시행착오를 겪었습니다. 단순히 기능만 구현하는 것이 아니라, 도메인 주도 설계 관점에서 게시글을 정의하고 ERD 설계를 통해 다른 엔티티와 관계를 구조적으로 연결하면서 데이터 흐름을 깊이 이해할 수 있었습니다. 또한 MyBatis와 JPA를 병행해 사용하며 각 기술의 장단점을 직접 체감했고, 그 과정에서 매핑 오류와 쿼리 처리 문제를 해결해 나가며 문제 해결 능력을 키울 수 있었습니다. MSA 구조 안에서 게시글 서비스를 독립적으로 설계하면서, 서비스 간 통신과 모듈 간 결합도를 어떻게 최소화할지 고민했던 경험도 값졌습니다. 이는 단순 구현을 넘어 아키텍처 차원에서 사고하는 훈련이 되었고, 확장성과 유지보수성을 고려하는 시야를 넓혀주었습니다. 이번 프로젝트에서 얻은 가장 큰 성과는 ‘기술을 배우는 방법’과 ‘스스로 성장할 수 있다는 자신감’이었습니다. 오류와 버그를 해결하는 과정이 힘들었지만, 그 과정에서 문제를 끝까지 파고드는 끈기와 학습 능력을 기를 수 있었습니다. 앞으로도 이번 경험을 발판 삼아, 더 나은 코드를 작성하고 더 깊이 있는 설계를 고민하는 개발자로 성장해 나가고자 합니다. |
